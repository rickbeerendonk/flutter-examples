...
class _AppState extends State<App> {
  ...
  void insertTop() {
    setState(() {
      final newItem = (items.length + 1).toString();
      items = [newItem, ...items];
      // Create controller for new item
      controllers[newItem] = TextEditingController();
    });
  }
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  key: ValueKey(
                      items[index]), // Use item value as key to avoid issues
                  title: TextField(
                    controller: controllers[items[index]],
